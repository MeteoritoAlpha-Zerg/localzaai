...
class TestListHelpers(unittest.TestCase):
    def test_simple_find_insertion_index(self):
        ...
        assert find_insertion_index(list(reversed(sorted_list)), 4, lambda x: x, True) == 0

        assert find_insertion_index(sorted_list, 2, lambda x: x, False) == 1
        assert find_insertion_index(list(reversed(sorted_list)), 2, lambda x: x, True) == 1

    def test_key_find_insertion_index(self):
        sorted_list = [{"not-id": 10}, {"id": 1}, {"id": 5}]

        assert find_insertion_index(sorted_list, {"id": 3}, lambda x: x.get("id", 0), False) == 2
        assert find_insertion_index(sorted_list, {"uh oh": 3}, lambda x: x.get("id", 0), False) == 1
        assert find_insertion_index(list(reversed(sorted_list)), {"id": 3}, lambda x: x.get("id", 0), True) == 1
    ...
    def test_insert_and_maintain_order(self):
        ...
        new_list = sorted_list.copy()
        insert_and_maintain_order(new_list, {"uh oh": 3}, lambda x: x.get("id", 0), False)
