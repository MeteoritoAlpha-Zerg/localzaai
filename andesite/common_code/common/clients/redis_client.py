...
class RedisConfig(BaseSettings):
    ...
    def validate_connection_string(self) -> Self:
        if not self.host or not self.port:
            raise ValueError(f"Cannot form a valid connection string from {self.host} and {self.port}")
        return self
...
class RedisClient:
    ...
    async def initialize(cls, cfg: RedisConfig) -> None:
        cls._client = AsyncRedis.from_url(cfg.redis_url, socket_timeout=60, socket_connect_timeout=30)
        cls._sync_client = SyncRedis.from_url(cfg.redis_url, socket_timeout=60, socket_connect_timeout=30)
