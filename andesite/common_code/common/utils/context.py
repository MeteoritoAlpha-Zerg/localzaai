...
TMessageAttributes = dict[str, dict[str, str | None]]

context_request_id = ContextVar("request_id", default="unknown")
context_user_id = ContextVar("user_id", default="unknown")
context_conversation_id: ContextVar[str | None] = ContextVar("conversation_id", default=None)
context_request_path: ContextVar[str | None] = ContextVar("request_path", default=None)
context_request_method: ContextVar[str | None] = ContextVar("request_method", default=None)
context_request_timer: ContextVar[TotalAccess | None] = ContextVar("request_timer", default=None)
context_token_count = ContextVar("token_count", default=0)
context_mock_mode = ContextVar("mock_mode", default=False)
context_llm_model_id: ContextVar[str | None] = ContextVar("llm_model", default=None)
context_experimental_chat = ContextVar("experimental_chat", default=False)
context_experimental_document_processing = ContextVar("experimental_document_processing", default=False)


@overload
def get_header_value(headers: Headers, key: str, default: None = None) -> str | None: ...
...
def get_header_value(headers: Headers, key: str, default: str | None = None):
    """
...
def set_context_from_message_attributes(message_attributes: TMessageAttributes):
    ...
    context_request_path.set(message_attributes.get("X-Request-Path", {}).get("StringValue"))
    context_request_method.set(message_attributes.get("X-Request-Method", {}).get("StringValue"))
    ...
    context_mock_mode.set(message_attributes.get("X-Mock-Mode", {}).get("StringValue", "") == "True")
    context_experimental_chat.set(message_attributes.get("X-Experimental-Chat", {}).get("StringValue", "") == "True")
    context_experimental_document_processing.set(
        message_attributes.get("X-Experimental-Document-Processing", {}).get("StringValue", "") == "True"
    )
...
def with_context_from_message_attributes(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        message_attributes = kwargs.get("message_attributes", get_message_attributes_new())
...
def set_context_from_http_request(request: Request, logger: logging.Logger):
    ...
    if get_header_value(headers, "X-Experimental-Chat", "false").strip().lower() == "true":
        context_experimental_chat.set(True)

    if get_header_value(headers, "X-Experimental-Document-Processing", "false").strip().lower() == "true":
...
def set_span_attributes(span: Span, message: dict):
    if span and span.is_recording():
        ...
        if context_conversation_id.get() is not None:
            span.set_attribute("X-Conversation-Id", context_conversation_id.get())  # type: ignore[arg-type]
