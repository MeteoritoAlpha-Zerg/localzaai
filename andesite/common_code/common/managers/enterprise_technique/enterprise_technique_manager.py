...
class EnterpriseTechniqueManager(CollectionManager):
    ...
    async def load_initial_techniques_async(self, path: Path, reload: bool = False):
        if not path.exists() or not path.is_dir():
            logger().error(f'Default technique path "{path}" does not exist or is not a directory')
            raise EnterpriseTechniqueManagerException(f'Default technique path "{path}" does not exist')

        if not reload and str(path) in self._loaded_paths:
            logger().info(f"Path '{path}' already loaded and reload is False. Skipping load.")
            return
    ...
    def load_initial_techniques(self, path: Path, reload: bool = False):
        ...
        :return: None
        """
        return self._get_event_loop().run_until_complete(self.load_initial_techniques_async(path, reload))
    ...
    async def initialize(self, storage_collection: AgnosticCollection | None = None, data=None):
        if self._collection is None:
    ...
    @check_client_initialization
    async def list_techniques(self, page: int, page_size: int) -> MitreEnterpriseTechniquePage:
        ...
        return MitreEnterpriseTechniquePage(page=page, page_size=page_size, total_count=doc_count, results=docs)

    @check_client_initialization
    async def get_highest_technique_priority(self, mitre_technique_ids: list[str]) -> float | None:
        """
    ...
    @check_client_initialization
    async def get_technique_priorities(self, page_size: int | None = None) -> MitreTechniquePriorities:
        ...
        async for document in cursor:
            techniques.append(MitreTechniquePriority(id=document["tid"], priority=document["priority"]))
        return MitreTechniquePriorities(techniques=techniques)

    @check_client_initialization
    async def create_technique(self, doc: MitreEnterpriseTechnique) -> dict | None:
    ...
    @check_client_initialization
    async def update_technique(self, technique_id: str, doc: MitreEnterpriseTechnique) -> str:
