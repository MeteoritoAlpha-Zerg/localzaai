...
class DatasetStructureManager:
    ...
    async def get_all_dataset_structures_async(self, connector: str) -> list[DatasetStructure]:
        if self._storage_collection is None:
            raise DatasetStructureException(
                "Unable to get dataset structure because no storage collection was initialized."
            )
    ...
    def get_dataset_structure(self, connector: str, dataset: str) -> list[DatasetStructure]:
        """
    ...
    async def delete_dataset_structures_async(self, connector: str, datasets: list[str]):
        """
    ...
    async def set_dataset_structure_async(self, dataset_structure: DatasetStructure) -> DatasetStructure:
        """
        ...
        try:
            upserted_document: dict = await self._storage_collection.find_one_and_update(
                filter={
