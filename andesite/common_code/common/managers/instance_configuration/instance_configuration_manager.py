...
default_settings: dict[InstanceConfigurationSettingEnum, InstanceConfigurationSetting] = {
    ...
    InstanceConfigurationSettingEnum.ALERT_GROUPING_LOOKBACK_PERIOD_IN_SECONDS: InstanceConfigurationSetting(
        setting_name=InstanceConfigurationSettingEnum.ALERT_GROUPING_LOOKBACK_PERIOD_IN_SECONDS,
        setting_value=86400,
    ),
    InstanceConfigurationSettingEnum.MIN_ALERTS_PER_ANOMALY_GROUP: InstanceConfigurationSetting(
        setting_name=InstanceConfigurationSettingEnum.MIN_ALERTS_PER_ANOMALY_GROUP,
        setting_value=8,
    ),
    InstanceConfigurationSettingEnum.ALERT_ANOMALY_LOOKBACK_PERIOD_IN_SECONDS: InstanceConfigurationSetting(
        setting_name=InstanceConfigurationSettingEnum.ALERT_ANOMALY_LOOKBACK_PERIOD_IN_SECONDS,
        setting_value=86400 * 12,
    ),
}
...
class InstanceConfigurationManager:
    ...
    async def upsert_configuration_setting_async(
        ...
        try:
            ...
            updated_rule = InstanceConfigurationSetting.from_mongo(updated_mongo_document)
            if updated_rule is None:
                raise InstanceConfigurationException(f"Unable to update document with name '{setting.setting_name}'")
    ...
    async def insert_configuration_setting_async(
        ...
        try:
            ...
            if insert_result.inserted_id is None:
                raise InstanceConfigurationException(f"Unable to insert setting with name '{setting.setting_name}'")
            ...
            logger().exception(f"An error occurred while setting instance setting with name '{setting.setting_name}'")
            raise
    ...
    async def initialize(self, storage_collection: AgnosticCollection | None = None):
        ...
        :return: None
        """
        missing_settings = set(InstanceConfigurationSettingEnum) - set(default_settings.keys())
        if missing_settings:
