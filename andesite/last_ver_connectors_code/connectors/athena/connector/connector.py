...
async def _get_query_target_options(config: AthenaConnectorConfig) -> ConnectorQueryTargetOptions:
    CATALOG = "catalog"
    DATABASE = "database"
    TABLES = "tables"
    WORKGROUP = "workgroup"
    ...
    try:
        catalog_response = ListDataCatalogsResponse.model_validate(
            client.list_data_catalogs()
        )
        catalogs: list[str] = [
            catalog.CatalogName for catalog in catalog_response.DataCatalogsSummary
        ]
        ...
        for catalog in catalogs:
            ...
            database_response = ListDatabaseResponse.model_validate(
                client.list_databases(CatalogName=catalog)
            )
            databases = [
                database.Name for database in database_response.DatabaseList
            ]
            ...
            for database in databases:
                ...
                table_selector.values = [
                    table.Name for table in table_response.TableMetadataList
                ]
                # TODO: this type error is complaining bc values can be a list and a string cannot index into a list. However, we know this is a dict from when we define the selector initially
        ...
        logger().exception(
            "Failed to get ScopeTargetSelector options from Athena: %s", str(e)
        )
        raise e
...
def _get_tools(display_name: str, config: AthenaConnectorConfig, target: AthenaTarget, token: SecretStr | None) -> list[Tool]:
...
async def _merge_data_dictionary(
    config: AthenaConnectorConfig, token: SecretStr | None, existing_dataset_descriptions: list[DatasetDescription], path_prefix: Optional[list[str]] = []
    ...
    for catalog in catalogs:
        ...
        catalog_description = (
            existing_catalog_description.description
            if existing_catalog_description
            else ""
        )
        ...
        for database in databases:
            ...
            existing_database_description = next(
                (
                    ...
                    if dd.path == [catalog, database]
                ),
                None,
            )
            database_description = (
                existing_database_description.description
                if existing_database_description
                else ""
            )
            data_dictionary.append(
                DatasetDescription(
            ...
            for table in tables:
                ...
                existing_table_description = next(
                    (
                        dd
                        for dd in existing_dataset_descriptions
                        if dd.path == [catalog, database, table]
                    ),
                    None,
                )
                table_description = (
                    existing_table_description.description
