...
class SplunkInstance(QueryInstance):
    ...
    def _init_sslcontext(self) -> ssl.SSLContext | None:
        if not self._mtls_client_cert_path or not self._mtls_client_key_path:
            logger().warn("attempting to initialize splunk sslcontext, but no cert/key paths provided")
        ...
        if self._mtls_client_cert_data and self._mtls_client_key_data:
            ...
            with open(cert_file, "w") as cert:
                cert.write(self._mtls_client_cert_data.get_secret_value())
            ...
            with open(key_file, "w") as key:
                key.write(self._mtls_client_key_data.get_secret_value())
    ...
    @retry_splunk_auth_error
    @tracer.start_as_current_span("_raw_spl_query")
    def _raw_spl_query(
        self,
        spl: str,
        earliest: str | None = "-1h",
        latest: str | None = "now",
        return_preview_events: bool = False,
    ):
        if self._client is None:
            return [{"error": "Not connected to Splunk instance"}]

        logger().info("Querying Splunk from %s to %s: %s", earliest, latest, spl)
        kwargs = {
            "search_mode": "normal",
            "output_mode": "json",
            "count": 0,
        ...
        # NOTE: `oneshot` is a blocking/synchronous query, if this causes issues with splunk look into changing this back to `export`
        results_stream = self._client.jobs.oneshot(spl, **kwargs)
        reader = splunklib_results.JSONResultsReader(results_stream)
        ...
        for result in reader:
            ...
            if not reader.is_preview or return_preview_events:
    ...
    _raw_spl_query_async = async_wrap(_raw_spl_query)
    ...
    async def spl_query(
        ...
        return await self._raw_spl_query_async(spl, earliest, latest, return_preview_events)

    async def execute_query(self, query: str, earliest: str | None = "-1h", latest: str | None = "now", limit: int | None = 100) -> list[dict[str, Any]]:
        return await self.spl_query(spl_query=query, earliest=earliest, latest=latest, return_preview_events=False, limit=limit)
    ...
    async def fetch_alerts(self, earliest: str, latest: str) -> list[dict[str, Union[str, list[str]]]]:
        ...
        alerts = await self.spl_query(
            f"search {notable_index} | table* | eval andesite_time=_time",
            earliest,
            latest,
            limit=5000,
        )
    ...
    def _get_notable_index(self):
        return "`notable`" if self._es else f"index={self._notable_index}"
