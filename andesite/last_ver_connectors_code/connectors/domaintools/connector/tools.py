...
class DomainToolsConnectorTools(ConnectorToolsInterface):
    """
    ...
    def __init__(
        ...
        """
        self.api = API(
            domain_tools_config.api_username.get_secret_value(),
            domain_tools_config.api_key.get_secret_value(),
        )
        super().__init__(ConnectorIdEnum.DOMAINTOOLS, target=target, connector_display_name=connector_display_name)

    def get_tools(self) -> list[Tool]:
        tools: list[Tool] = []
        tools.append(
            Tool(
                name="get_iris_investigate",
                connector=self._connector_display_name,
                execute_fn=self.get_iris_investigate_async,
            )
        )
        tools.append(
            Tool(
    ...
    class GetIrisInvestigateSchema(BaseModel):
        """
    ...
    @tracer.start_as_current_span(name="get_iris_investigate_async")
    async def get_iris_investigate_async(
        self, input: GetIrisInvestigateSchema
    ) -> QueryResultMetadata:
                    ...
                    if (
                        ...
                        if 2 <= value["count"] <= 500:
                            pivot_candidates.append(
                                (full_key, value["value"], value["count"])
                            )
                    else:
                        pivot_candidates.extend(
                            find_attributes_with_count(value, full_key)
                        )
            elif isinstance(data, list):
                for element in data:
                    pivot_candidates.extend(
                        find_attributes_with_count(element, parent_key)
                    )
        ...
        try:
            logger().info(
                f"Querying DomainTools Iris Investigate for domain: {input.domain}"
            )
            response = await self.api.iris_investigate(input.domain)
            domain_info = response.response().get("results", {})
            pivot_objects = find_attributes_with_count(domain_info)
            if pivot_objects:
                pivot_flag = f"Suggest pivot on attributes: {pivot_objects}"
            else:
                pivot_flag = "No pivot suggested"
        except Exception as e:
            error_message = f"Error querying DomainTools Iris Investigate: {str(e)}"
    ...
    def get_pivot_domain_filters(
        self, input: GetPivotDomainFiltersSchema
    ) -> dict[str, str]:
        return {
    ...
    class GetPivotDomainsInput(BaseModel):
        ...
        filters: dict[str, Any] = Field(
            description="A dictionary of filters to use in the query."
        )

    @tracer.start_as_current_span(name="get_pivot_domains_async")
    async def get_pivot_domains_async(
        self, input: GetPivotDomainsInput
    ) -> QueryResultMetadata:
        try:
            logger().info(
                f"Querying DomainTools Iris Investigate for pivot domains with filters: {input.filters}"
            )
            response = await self.api.iris_investigate(**input.filters)
            data = response.response()
            results = data.get("results", [])
            pivot_domains = [item.get("domain") for item in results]
            domain_list = [item for item in pivot_domains]
        except Exception as e:
