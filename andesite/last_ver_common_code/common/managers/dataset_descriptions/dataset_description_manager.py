...
class DatasetDescriptionManager:
    ...
    async def load_initial_descriptions_async(self, path: Path):
        ...
        if not path.exists() or not path.is_dir():
            logger().error(
                f'Default dataset description path "{path}" does not exist or is not a directory'
            )
            raise DatasetDescriptionException(
                f'Default prompt path "{path}" does not exist'
            )
        ...
        for file in path.glob("*.csv"):
            try:
                ...
                logger().error(
                    "Encountered error when loading initial data dictionary", exc_info=e
                )
                continue
    ...
    async def get_dataset_descriptions_async(
        ...
        pipeline: list[dict[str, Any]] = [
            {
                "$match": {
                    ...
                    **(
                        {"path": path_prefix}
                        if exact_path
                        else DatasetDescription.path_to_mongo_filter(
        ...
        for dataset_description in dataset_descriptions:
            dataset_description_response = DatasetDescription.from_mongo(
                dataset_description
            )
    ...
    def get_dataset_descriptions(
        self, connector: str, path_prefix: Optional[list[str]] = []
    ) -> list[DatasetDescription]:
        """
        ...
        :raises DatasetDescriptionException: If the dataset description is not found.
        """
        dataset_descriptions: list[DatasetDescription] = (
            self._get_event_loop().run_until_complete(self.get_dataset_descriptions_async(connector=connector, path_prefix=path_prefix))
        )
    ...
    @check_client_initialization
    async def set_dataset_description_async(
