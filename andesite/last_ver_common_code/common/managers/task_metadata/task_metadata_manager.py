...
class TaskMetadataManager:
    ...
    async def get_task_metadata_async(
        ...
        if self._storage_collection is None:
            raise TaskMetadataException(
                "Unable to get task metadata because no storage collection was initialized."
            )
    ...
    async def end_task_metadata_async(
        self, task_id: str, status: TaskStatusEnum
    ) -> TaskMetadata | None:
        """
        ...
        try:
            ...
            raise TaskMetadataException(f"Unable to end task with task_id '{task_id}'")

        if completed_task is None:
            # occurs when the task metadata doesn't exist or is already reset
            logger().info(
                "Document with task_id '%s' was already ended or never existed", task_id
            )

        return completed_task

    @tracer.start_as_current_span("upsert_task_metadata_async")
    async def upsert_task_metadata_async(
        ...
        try:
            ...
            if updated_task_metadata is None:
                raise TaskMetadataException(
                    f"Unable to update document with id '{task_metadata.task_id}'"
                )
            ...
            logger().error(
                f"An error occurred while setting task metadata with id '{task_metadata.task_id}': {e}"
            )
            raise e
