...
async def test_list_techniques(etm_instance, mongo_collection):
    find_mock = Mock()
    find_mock.return_value.skip.return_value.limit.return_value.to_list = AsyncMock(
        return_value=[technique_document]
    )
...
async def test_create_technique(
    etm_instance, mongo_collection, acknowledgment, expected_result
):
    mongo_collection.insert_one = AsyncMock(
        return_value=Mock(acknowledged=acknowledgment)
    )
...
async def test_update_technique(etm_instance, mongo_collection):
    ...
    mock.assert_awaited_once_with(
        {"tid": technique_model.tid}, {"$set": technique_document}, return_document=True
    )
...
async def test_update_technique_batch(etm_instance, mongo_collection):
    ...
    result = await etm_instance.update_technique_batch(
        techniques=[technique_model] * document_count
    )
    mock.assert_awaited_once_with(
        [
            UpdateOne(
