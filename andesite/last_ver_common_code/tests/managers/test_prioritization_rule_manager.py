...
async def test_get_all_rules(
    ...
    metadata = PrioritizationRule(
        rule_name="name", field_name="field", field_regex="regex", priority_boost=1
    )
...
async def test_upsert(prioritization_rule_manager: PrioritizationRuleManager) -> None:
    ...
    rule = PrioritizationRule(
        rule_name=rule_name, field_name="field", field_regex="regex", priority_boost=1
    )
    await prioritization_rule_manager.upsert_prioritization_rule_async(rule)
    saved_rule = await prioritization_rule_manager.get_prioritization_rule_async(
        rule_name
    )
    ...
    new_rule = await prioritization_rule_manager.upsert_prioritization_rule_async(
        saved_rule
    )
...
async def test_delete(prioritization_rule_manager: PrioritizationRuleManager) -> None:
    ...
    await prioritization_rule_manager.delete_prioritization_rules_async(
