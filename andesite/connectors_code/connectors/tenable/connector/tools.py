...
class TenableConnectorTools(ConnectorToolsInterface[TenableSecrets]):
    """
    ...
    def __init__(
        ...
        """
        super().__init__(ConnectorIdEnum.TENABLE, target=target, secrets=secrets)

    def get_tools(self) -> list[Tool]:
        tools: list[Tool] = []
        tools.append(
            Tool(
                connector=ConnectorIdEnum.TENABLE,
            ...
            try:
                response = await client.get(
                    ...
                    headers={
                        ...
                        "X-ApiKeys": f"accessKey={self._secrets.access_key.get_secret_value()};secretKey={self._secrets.secret_key.get_secret_value()}",
                    },
                )
                response.raise_for_status()
            except HTTPStatusError as exc:
                error_message = f"API Error. Code={exc.response.status_code},Response={exc.response.text}"
                if has_filters:
    ...
    class GetAssetFiltersInput(BaseModel):
        ...
        ALWAYS USE THIS BEFORE THE get_assets() TOOL IN CASE THE ASSET NAME IS NOT IN THE EXPECTED FORMAT!!!
        """

        pass

    @tracer.start_as_current_span("get_asset_filters_async")
    async def get_asset_filters_async(self, input: GetAssetFiltersInput) -> QueryResultMetadata:
        (_, metadata) = await self.call_tenable_api(path="filters/workbenches/assets", has_filters=False)
        return metadata
    ...
    async def get_vulnerability_filters_async(self, input: GetVulnerabilityFiltersInput) -> QueryResultMetadata:
        (_, metadata) = await self.call_tenable_api(path="filters/workbenches/vulnerabilities", has_filters=False)
        return metadata
    ...
    class GetAssetsInput(BaseModel):
        ...
        Before retrieving a specific asset, ALWAYS USE THE get_asset_filters() FIRST EVEN MID-CONVERSATION!!!
        If no results for a host with domain, try just the NETBIOS name.
        ...
        filters: Optional[list[tuple[Any, ...]]] = Field(
    ...
    async def get_assets_async(
        ...
        query_params.extend(TenableConnectorTools._get_filters_query_params(filters, filter_type))

        response, metadata = await self.call_tenable_api(
            path="workbenches/assets", query_params=query_params, has_filters=True
        )
        additional_context = None
        if response.get("total", 0) > 100 and "assets" in response and isinstance(response["assets"], list):
        ...
        return ToolResult(result=metadata, additional_context=additional_context)
    ...
    async def get_asset_info_async(self, input: GetAssetInfoInput) -> QueryResultMetadata:
    ...
    async def get_asset_vulnerabilities_async(
        ...
        additional_context = None
        if (
            response.get("total_vulnerability_count", 0) > 100
            and "vulnerabilities" in response
            and isinstance(response["vulnerabilities"], list)
        ):
    ...
    async def get_vulnerable_assets_async(
        ...
        additional_context = None
        if response.get("total_asset_count", 0) > 100 and "assets" in response and isinstance(response["assets"], list):
            response["assets"] = response["assets"][:100]
            metadata.results = [
                [
                    json.dumps(response),
