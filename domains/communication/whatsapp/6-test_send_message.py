...
async def test_send_message(zerg_state=None):
    """Test WhatsApp message sending from selected phone number"""
    ...
    from connectors.whatsapp.tools import WhatsAppConnectorTools, SendWhatsAppMessageInput
    ...
    # grab the send_whatsapp_message tool and execute it with test message
    send_whatsapp_message_tool = next(tool for tool in tools if tool.name == "send_whatsapp_message")
    test_message = f"Test WhatsApp message from connector at {datetime.now().isoformat()}"
    test_to_number = "+1234567890"  # Use a test number or verified contact
    
    send_result = await send_whatsapp_message_tool.execute(
        phone_number_id=from_phone_number_id,
        to=test_to_number,
        message_type="text",
        text_body=test_message
    )
    ...
    # Verify that the WhatsApp message was sent successfully
    assert isinstance(send_response, dict), "send_response should be a dict"
    
    # Verify essential WhatsApp message response fields
    ...
    print(f"Successfully sent WhatsApp message from phone number ID {from_phone_number_id} to {test_to_number}")
