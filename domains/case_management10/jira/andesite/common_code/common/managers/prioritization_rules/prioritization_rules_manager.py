...
class PrioritizationRuleManager:
    ...
    async def load_initial_rules_async(self, path: Path, reload: bool = False):
        if not path.exists() or not path.is_dir():
            logger().error(f'Default rule path "{path}" does not exist or is not a directory')
            raise PrioritizationRuleException(f'Default rule path "{path}" does not exist')
    ...
    def load_initial_rules(self, path: Path, reload: bool = False):
        ...
        :return: None
        """
        return self._get_event_loop().run_until_complete(self.load_initial_rules_async(path, reload))
    ...
    async def delete_prioritization_rules_async(self, rule_names: list[str]):
        ...
        delete_result = await self._storage_collection.delete_many({"rule_name": {"$in": rule_names}})
    ...
    async def insert_prioritization_rule_async(self, rule: PrioritizationRule) -> PrioritizationRule:
        """
        ...
        try:
            ...
            if insert_result.inserted_id is None:
                raise PrioritizationRuleException(f"Unable to insert rule with name '{rule.rule_name}'")
            ...
            logger().exception(f"An error occurred while setting prioritization rule with name '{rule.rule_name}'")
            raise

    @tracer.start_as_current_span("upsert_prioritization_rule_async")
    async def upsert_prioritization_rule_async(self, rule: PrioritizationRule) -> PrioritizationRule:
        """
        ...
        try:
            ...
            if updated_rule is None:
                raise PrioritizationRuleException(f"Unable to update document with name '{rule.rule_name}'")
            ...
            logger().error(f"An error occurred while setting prioritization rule with name '{rule.rule_name}': {e}")
            raise e
